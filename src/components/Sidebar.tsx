...
export const Sidebar = ({ isSidebarOpen }: { isSidebarOpen: boolean }) => {
    ...
    return (
        <div className={`flex lg:hidden flex-col fixed top-18 left-0 h-screen pb-[200px] overflow-y-auto
         w-full bg-[#FFFFFFF2] transform transition-transform  duration-600 ease-in-out z-40 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} backdrop-blur-lg`}>
                ...
                <ul className="mt-4 space-y-4 text-lg">
                    {
                        sidebarData.map((content, index) => (
                            <div
                                key={index}
                                className="justify-between items-center px-[20px] bg-[#F8F8F9] min-w-[300px] py-[20px] rounded-2xl"
                                onClick={() => toggleAccordion(index)}
                            >
                                <button className="flex justify-between w-full cursor-pointer">
                                    <span className="text-[#002444] font-light">
                                        {content.title === "About" ? <Link href="/about">About</Link> : content.title}
                                    </span>
            ...
            <div className="flex flex-col justify-between gap-[15px] items-center px-[20px] mt-[30px]">
                <button
                    className="bg-[#252525] flex flex-row text-white px-10 py-8 w-full rounded-full cursor-pointer"
                >
                    <div className="flex flex-row items-center justify-center w-full gap-[10px]">
                        <span className="text-center">
                            <FaApple className="text-white" size={30} />
                        </span>
                        <span className='text-[20px] text-center font-medium'>Coming Soon to iPhone</span>
                    </div>
                </button>
                <button
                    className="bg-[#086C30] flex flex-row text-white px-10 py-8 w-full rounded-full cursor-pointer"
                >
                    <div className="flex flex-row items-center justify-center w-full gap-[10px]">
                        <span className="text-center">
                            <FaGooglePlay className="text-white" size={30} />
                        </span>
                        <span className='text-[20px] text-center font-medium'>Coming Soon to Android</span>
                    </div>
                </button>
                <Link href="/signup"
                    className="bg-[#06284C] flex flex-row text-white px-10 py-8 w-full rounded-full cursor-pointer"
                >
                    <div className="flex flex-row items-center justify-center w-full gap-[10px]">
                        <span>
                            <img
                                src='https://ik.imagekit.io/rwgk2b4rf/svgexport-7%20(1).svg'
                                alt=""
                            />
                        </span>
